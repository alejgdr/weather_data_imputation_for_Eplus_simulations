...
def seasonal_pre_process(in_size,out_size,esoru,scaler,scaler2,inputs,outputs,training_step,season_size): #scaler fit is here #fits the scaler to the training data , scales  the data, and arrange it in arrays to feed the model
     ...
     dump(scaler, open('03_scalers/x_scalerv02.pkl','wb'))
     dump(scaler2,open('03_scalers/y_scalerv02.pkl','wb'))
     pre_array=[]
     pre_arrax=[]
...
initial_epochs=1000
name_model='train_seasonal_layers_3_3_1_out_s_'+str(out_size)+'in_s_'+str(in_size)+'_lr_'+str(lr)+'_'+(output[0])+'_BS'+str(bs)
#*******************************************************************************************************
print('batch_size:',bs)
print(name_model+'.h5'+ '\n')
